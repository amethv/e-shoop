#session / permite que cuando se abra una sesión en el navegador y se autentifica, esto se haga una sola vez, sin pedir nuevamente usuario y contrasena
...
def home():
    ...
    #Render_template / para llamar a la plantilla y se pueda renderizar
...
def detalle_producto(id):
    #Error 404 / El navegador no encuentra lo que se le está mandando
...
# ── CARRITO ───────────────────────────────────────────────────────
# Cookie es un fragmento de datos que se guarda en el navegador mientras se está iniciada sesión
...
def agregar_carrito(id):
    ...
    carrito = session.get('carrito', {})  #get me indica que ahí se está guardando en una cookie
    clave   = str(id)
...
def eliminar_carrito(id):
    #Obteniendo la cookie del carrito
    carrito = session.get('carrito', {})
    carrito.pop(str(id), None)
    #Guardando el carrito de compras en la cookie del navegador
    session['carrito'] = carrito
...
@login_required #SE REQUIERE UN INICIO DE SESIÓN PARA REALIZAR ESTO
def pago():
    carrito = session.get('carrito', {})
    ...
    if request.method == 'POST':
        ...
        for prod_id, cantidad in carrito.items():
            ...
            producto = Producto.query.get(int(prod_id))
            #siempre se debe ir verificando el stock
        ...
        #Solamente aquí se está guardando en la base de datos
        db.session.commit()
    ...
    #GET ES CUANDO SE REALIZA UNA REVISION DE LO QUE SE QUIERE COMPRAR 
    items = []
    total = 0
    for prod_id, cantidad in carrito.items():
        producto = Producto.query.get(int(prod_id))
